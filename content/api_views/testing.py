...
from content.models.grammar import (
    GrammarConcept, ChunkGrammarFocus, GrammarQuestion
)
from content.models.punctuation import (
    PunctuationMark, ChunkPunctuationFocus, PunctuationQuestion
)
from content.models.comprehension import (
    ChunkComprehensionFocus, ComprehensionQuestion, BloomLevel
)

# Writing — new three-tier architecture
# Unit test writing questions now draw from WritingStageContent
# rather than the old UnitWritingTask / WritingPrompt models.
...
class UnitTestSessionViewSet(BaseViewSet, UserFilterMixin):
    ...
    def validate_new_session_allowed(self, user, unit_id):
        ...
        if attempt_count >= self.max_attempts_per_unit:
            raise ValidationError(
                f"Maximum {self.max_attempts_per_unit} attempts reached "
                f"for this unit"
            )
        in_progress = UnitTestSession.objects.filter(
            user=user, unit_id=unit_id, completed_at__isnull=True
        ).exists()
        if in_progress:
            raise ValidationError(
    ...
    def generate_test_questions(self, unit, attempt_number):
        ...
        Writing questions now draw from WritingStageContent.
        """
        questions = []
        ...
        if vocab_items.exists():
            selected = random.sample(
                list(vocab_items), min(5, vocab_items.count())
            )
        ...
        if grammar_focuses.exists():
            ...
            if grammar_questions.exists():
                selected = random.sample(
                    list(grammar_questions), min(5, grammar_questions.count())
                )
                for q in selected:
                    questions.append({
                        'domain':            'grammar',
        ...
        if punct_focuses.exists():
            ...
            if punct_questions.exists():
                selected = random.sample(
                    list(punct_questions), min(3, punct_questions.count())
                )
                for q in selected:
                    questions.append({
                        'domain':              'punctuation',
        ...
        if comp_focuses.exists():
            ...
            if comp_questions.exists():
                selected = random.sample(
                    list(comp_questions), min(4, comp_questions.count())
                )
                for q in selected:
                    questions.append({
        ...
        # Writing prompt (10%) — now from WritingStageContent
        # Use the Produce prompt from complete stage content for this unit
        writing_contents = WritingStageContent.objects.filter(
            unit=unit,
            is_complete=True,
        ).select_related('stage').order_by('stage__number')

        if writing_contents.exists():
            selected = random.sample(
                list(writing_contents), min(1, writing_contents.count())
            )
            for content in selected:
                questions.append({
                    'domain':         'writing',
                    'question_type':  'open_ended',
                    'question_text':  content.produce_prompt,
                    'options':        [],
                    'correct_answer': '',  # Open-ended — teacher evaluated
                    'difficulty':     3,
                    'points':         5,  # Writing questions worth more
                })

        # Pronunciation (5%) — placeholder
        pron_focuses = PronunciationFocus.objects.filter(chunk__in=chunks)
        if pron_focuses.exists():
            pass  # Pronunciation questions added in future
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            return Response(
                {'success': False, 'error': 'Failed to start test session'},
    ...
    def results(self, request, pk=None):
        ...
        results = {
            ...
            'time_taken_minutes':  (
                ...
                if session.time_taken_seconds else None
            ),
...
class UnitTestQuestionViewSet(BaseViewSet):
    ...
    def analytics(self, request, pk=None):
        ...
        analytics = {
            ...
            'average_time':  answers.aggregate(
                avg=Avg('time_taken_seconds')
            )['avg'],
            'by_session':    [],
        }

        sessions = answers.values_list(
            'question__session', flat=True
        ).distinct()
        for session_id in sessions[:10]:
            session_answers = answers.filter(
            ...
            analytics['by_session'].append({
                ...
                'accuracy':   (
                    session_correct / session_answers.count() * 100
                ),
            })
...
class UnitTestProgressViewSet(ProgressViewSet):
    ...
    def get_user_progress(self, user):
        ...
        return {
            ...
            'pass_rate': (
                (passed_sessions / completed_sessions * 100)
                if completed_sessions > 0 else 0
            ),
            'average_score': sessions.filter(
                completed_at__isnull=False
            ).aggregate(avg=Avg('score_percentage'))['avg'],
            'best_score': sessions.filter(
                completed_at__isnull=False
            ).aggregate(best=Max('score_percentage'))['best'],
            'unit_progress':      unit_progress,
            'domain_performance': avg_domain_scores,
            'last_activity': (
                sessions.order_by('-started_at').first().started_at
...
class TestGenerationViewSet(viewsets.GenericViewSet):
    ...
    def generate(self, request):
        ...
        if 'vocabulary' in include_domains:
            vocab_items = VocabularyItem.objects.filter(chunk__in=chunks)
            count    = min(
                questions_per_domain.get('vocabulary', 5), vocab_items.count()
            )
            ...
            for item in selected:
                questions.append({
                    ...
                    'question_type':  'mcq',
                    'question_text':  (
                        f"Select the correct meaning of '{item.word}'"
                    ),
                    'options':        [
    ...
    def migrate_vocabulary_tests(self, request):
        ...
        with transaction.atomic():
            for legacy in legacy_sessions:
                ...
                UnitTestSession.objects.create(
                    user_id=legacy.student_id,
                    unit=unit,
                    attempt_number=1,
